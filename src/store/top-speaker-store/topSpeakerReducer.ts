...
const initialState: ITopSpeakerStore = {
	...
	permanentDeletedTopSpeaker: null,
	loading: false,
	error: null,
};
...
const topSpeakerReducer = (state = initialState, action: any) => {
	switch (action.type) {
		...
		case `${TopSpeakerActionTypes.PERMANENT_DELETE_TOP_SPEAKER}_PENDING`:
		...
		case `${TopSpeakerActionTypes.PERMANENT_DELETE_TOP_SPEAKER}_FULFILLED`:
			let permanentDeletedTopSpeaker = action.payload.data;
			return { ...state, loading: false, permanentDeletedTopSpeaker };
		...
		case `${TopSpeakerActionTypes.PERMANENT_DELETE_TOP_SPEAKER}_REJECTED`:
			return {
				...state,
				loading: false,
